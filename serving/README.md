...
| `/image` | 1장 | 여러 장, figure 순서 유지, 드래그앤드롭·클립보드 |
...
**끌어다 놓기.** 터미널 창에 도면 파일을 떨어뜨리면 경로만 적힌 줄이 됩니다.
`/image` 를 앞에 붙이지 않아도 그대로 첨부됩니다. 여러 장을 한꺼번에 떨어뜨려도
되고, 순서는 떨어진 순서를 따릅니다. 터미널마다 형식이 달라 — Windows Terminal
은 공백이 든 경로를 따옴표로 감싸고, macOS·Linux 터미널은 역슬래시로 이스케이프
합니다 — 양쪽 다 풀도록 되어 있습니다.

**클립보드.** 터미널은 이미지 붙여넣기를 글자로 받지 못하므로, `Ctrl+V` 대신
`/clip` (또는 `/v`, 인자 없는 `/image`) 을 입력하면 클립보드를 직접 읽습니다.
캡처한 비트맵과 탐색기에서 복사한 파일 둘 다 됩니다. `pillow` 가 필요하고,
Linux 에서는 `xclip` 또는 `wl-paste` 가 있어야 합니다.

**경로 입력.** 위의 `/image <경로>` 형태입니다.
